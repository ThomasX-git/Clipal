...
func buildLaunchdPlist(binaryPath, configDir, stdoutPath, stderrPath string) string {
	...
	b.WriteString(`    <string>` + xmlEscape(darwinLaunchdLabel) + `</string>` + "\n")
	b.WriteString("\n")
	b.WriteString(`    <key>ProgramArguments</key>` + "\n")
	b.WriteString(`    <array>` + "\n")
	b.WriteString(`      <string>` + xmlEscape(binaryPath) + `</string>` + "\n")
	b.WriteString(`      <string>--config-dir</string>` + "\n")
	b.WriteString(`      <string>` + xmlEscape(configDir) + `</string>` + "\n")
	b.WriteString(`    </array>` + "\n")
	b.WriteString("\n")
	...
	if strings.TrimSpace(stdoutPath) != "" {
		...
		b.WriteString(`    <string>` + xmlEscape(strings.TrimSpace(stdoutPath)) + `</string>` + "\n")
	}
	if strings.TrimSpace(stderrPath) != "" {
		b.WriteString("\n")
		b.WriteString(`    <key>StandardErrorPath</key>` + "\n")
		b.WriteString(`    <string>` + xmlEscape(strings.TrimSpace(stderrPath)) + `</string>` + "\n")
	}
